  ...
  for (const f of files) {
    try {
      ...
      for (const item of abi) {
        ...
        map.set(selector, { name: item.name, inputs: item.inputs||[], outputs: item.outputs||[] });
      }
    } catch {}
  }
  return map;
...
function paramList(inputs) {
  return inputs.map((i,idx)=>`${i.type} ${i.name && i.name.trim() ? i.name : `p${idx}`}`).join(', ');
}
function returnList(outputs) {
  if (!outputs || outputs.length===0) return '';
  return ' returns (' + outputs.map((o,idx)=>`${o.type}${o.name?` ${o.name}`:''}`).join(', ') + ')';
}

function genFacet(facetName, entries) {
  const iface = entries.map(e => `    function ${e.name}(${paramList(e.inputs)}) external${returnList(e.outputs)};`).join('\n');
  const impl  = entries.map(e => {
    const r = returnList(e.outputs);
    const needsReturn = (e.outputs && e.outputs.length>0);
    const retZero = needsReturn ? ('(' + e.outputs.map(o=>{
...
async function main() {
  ...
  const planPath = process.argv.includes('--plan') ? process.argv[process.argv.indexOf('--plan')+1] : path.join(repo,'plan.json');
  const artifactsDir = process.argv.includes('--artifacts') ? process.argv[process.argv.indexOf('--artifacts')+1] : path.join(repo,'artifacts');

  if (!fs.existsSync(planPath)) throw new Error(`Missing plan file: ${planPath}`);
  if (!fs.existsSync(artifactsDir)) throw new Error(`Missing artifacts dir: ${artifactsDir}. Run "npx hardhat compile".`);
