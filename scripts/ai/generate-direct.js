...
try {
  ...
  aiEngine = { setOriginalContract(){} , mustFixLearner:{ validateCompliance:()=>({compliancePercentage:100,issues:[]})}};
  facetGenerator = {
    setOriginalContract(){},
...
${(cfg.functions||[]).map(fn=>`    function ${fn}() external {}`).join('\n')}
    ...
    generateManifest: (facets, meta)=>({ facets, deployment:{ strategy: meta?.deploymentStrategy||'sequential', networkSupport:['localhost'] }})
  };
}
...
async function main() {
  ...
  const facetsToGenerate = [
    ...
    { name:'InsuranceRewardsFacet', functions:['buyInsurance','submitClaim','processClaim','claimRewards'] },
  ];
  ...
  for (const f of facetsToGenerate) {
    ...
    results.push({ name:f.name, file: out, size:(code.length/1024).toFixed(1)+' KB', compliance:v.compliancePercentage, issues:v.issues.length, functions:f.functions });
    console.log(`✅ ${f.name} → ${out}`);
  }

  const manifest = facetGenerator.generateManifest(facetsToGenerate, { deploymentStrategy:'sequential' });
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));

  const archDir = path.join(repoRoot, 'arch');
  fs.mkdirSync(archDir, { recursive:true });
  fs.writeFileSync(path.join(archDir, 'ai_facets.index.json'),
    JSON.stringify({ facets: results.map(r=>({ name:r.name, file:r.file, functions:r.functions })) }, null, 2)
  );
