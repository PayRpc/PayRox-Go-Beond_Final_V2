...
function run() {
  ...
  for (const c of constructors) {
    ...
    if (node && node.body && node.body.statements && node.body.statements.length) {
      console.error(
        '❌ Constructor with body detected. Transformer will not automatically lift constructor bodies.',
      );
      console.error('File:', srcPath);
      console.error('Contract:', contractName);
      console.error(
        'Please extract constructor logic manually or re-run after implementing body lifting.',
      );
      process.exit(2);
    }
  }
