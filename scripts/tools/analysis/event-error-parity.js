...
function extractEventsAndErrors(file) {
  ...
  parser.visit(ast, {
    EventDefinition(node) { items.events.push(node.name); },
    UserDefinedError(node) { items.errors.push(node.name); }
  });
  return items;
}
...
function run(originalDir, diamondDir) {
  ...
  origFiles.forEach(f => {
    ...
    if (!fs.existsSync(dpath)) { report.push({file:f, status:'missing_in_diamond'}); return; }
...
if (process.argv.length < 3) { console.error('usage: node event-error-parity.js <original_dir> <diamond_dir>'); process.exit(1); }
