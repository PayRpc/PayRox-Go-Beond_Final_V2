...
const NETWORKS: NetworkRow[] = [
  // Tier 1
  { network: 'Ethereum Mainnet', chainId: 1, explorerUrl: 'https://etherscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Polygon', chainId: 137, explorerUrl: 'https://polygonscan.com', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Arbitrum One', chainId: 42161, explorerUrl: 'https://arbiscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Optimism', chainId: 10, explorerUrl: 'https://optimistic.etherscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Base', chainId: 8453, explorerUrl: 'https://basescan.org', status: 'production', create2Deployer: DEFAULT_EIP2470 },

  // Tier 2
  { network: 'BNB Smart Chain', chainId: 56, explorerUrl: 'https://bscscan.com', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Avalanche C-Chain', chainId: 43114, explorerUrl: 'https://snowtrace.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Fantom Opera', chainId: 250, explorerUrl: 'https://ftmscan.com', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Gnosis Chain', chainId: 100, explorerUrl: 'https://gnosisscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Celo', chainId: 42220, explorerUrl: 'https://celoscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },

  // Tier 3
  { network: 'Moonbeam', chainId: 1284, explorerUrl: 'https://moonbeam.moonscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Cronos', chainId: 25, explorerUrl: 'https://cronoscan.com', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Aurora', chainId: 1313161554, explorerUrl: 'https://aurorascan.dev', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Metis Andromeda', chainId: 1088, explorerUrl: 'https://explorer.metis.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Boba Network', chainId: 288, explorerUrl: 'https://bobascan.com', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Moonriver', chainId: 1285, explorerUrl: 'https://moonriver.moonscan.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Fuse', chainId: 122, explorerUrl: 'https://explorer.fuse.io', status: 'production', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Harmony One', chainId: 1666600000, explorerUrl: 'https://explorer.harmony.one', status: 'production', create2Deployer: DEFAULT_EIP2470 },

  // Testnets (Mumbai is legacy; prefer Amoy)
  { network: 'Ethereum Sepolia', chainId: 11155111, explorerUrl: 'https://sepolia.etherscan.io', status: 'testnet', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Polygon Amoy', chainId: 80002, explorerUrl: 'https://www.oklink.com/amoy', status: 'testnet', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Arbitrum Sepolia', chainId: 421614, explorerUrl: 'https://sepolia.arbiscan.io', status: 'testnet', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Base Sepolia', chainId: 84532, explorerUrl: 'https://sepolia.basescan.org', status: 'testnet', create2Deployer: DEFAULT_EIP2470 },
  { network: 'Optimism Sepolia', chainId: 11155420, explorerUrl: 'https://sepolia-optimism.etherscan.io', status: 'testnet', create2Deployer: DEFAULT_EIP2470 },

  // Local
  { network: 'Hardhat Local', chainId: 31337, explorerUrl: 'http://localhost:8545', status: 'local', create2Deployer: DEFAULT_EIP2470 },
];

// small utilities
function ensure0x(s: string) { return s.startsWith('0x') ? s : '0x' + s; }
...
async function loadArtifactInfo(name: string): Promise<{ bytecode?: string; compiler?: any } | null> {
  try {
    const art: any = await artifacts.readArtifact(name);
    const bytecode = art.bytecode && art.bytecode.startsWith('0x') && art.bytecode.length > 4 ? art.bytecode : undefined;
...
const argv: any = require('minimist')(process.argv.slice(2), {
  ...
  string: ['out', 'csv', 'md', 'factory-art', 'target-art', 'deployer', 'factory-salt', 'content', 'version', 'cross-nonce'],
  alias: { h: 'help', o: 'out' },
...
function usage() {
  ...
  console.log('Options:');
  console.log('  --out <file>           JSON manifest output (default ./manifests/cross-network-registry.json)');
  ...
  console.log('  --allow-fallback       Allow falling back to SaltViewFacet when target artifact missing');
...
async function main() {
  ...
  const solidityPack = utils.solidityPack || (utils as any).solidityPacked || EthersLib.solidityPack || ((types: any, values: any) => utils.defaultAbiCoder.encode(types, values));
  const getAddress = utils.getAddress || EthersLib.getAddress;

  const deployer = argv.deployer ? getAddress(argv.deployer) : getAddress(process.env.PRX_DEPLOYER_ADDR || DEFAULT_EIP2470);
  ...
  const factoryArtName = argv['factory-art'] || process.env.PRX_FACTORY_ART || 'DeterministicChunkFactory';
  const targetArtName = argv['target-art'] || process.env.PRX_TARGET_ART || 'PayRoxUniversalContract';

  // load artifact bytecode or use env-provided fallback bytecode
  const factoryInfo = process.env.PRX_FACTORY_BYTECODE ? { bytecode: ensure0x(process.env.PRX_FACTORY_BYTECODE), compiler: undefined } : await loadArtifactInfo(factoryArtName);
  const targetInfo = process.env.PRX_TARGET_BYTECODE ? { bytecode: ensure0x(process.env.PRX_TARGET_BYTECODE), compiler: undefined } : await loadArtifactInfo(targetArtName);
  ...
  if (!targetInitCode) {
    ...
    if (fbInfo?.bytecode) {
      const allow = argv['allow-fallback'] || (process.env.ALLOW_FALLBACK || 'false').toLowerCase() === 'true';
      console.warn(`⚠️ Target artifact '${targetArtName}' missing; found fallback '${fallback}'. allowFallback=${allow}`);
      if (!allow) {
        throw new Error(`Target artifact '${targetArtName}' missing; set PRX_TARGET_BYTECODE or pass --allow-fallback`);
      }
  ...
  function validateInitCode(name: string, initCode?: string) {
    ...
    if (len > EIP3860_MAX) throw new Error(`${name} init code size ${len} exceeds EIP-3860 ${EIP3860_MAX}`);
  }
  ...
  const defaultFactorySalt = keccak256(solidityPack(['string', 'string'], ['PayRoxFactory', version]));
  const factorySalt = argv['factory-salt'] ? hex32(argv['factory-salt']) : hex32(defaultFactorySalt);

  const factoryInitCodeHash = keccak256(factoryInitCode as string);
  const universalSalt = keccak256(solidityPack(['string', 'string', 'uint256', 'string'], ['PayRoxCrossChain', content, crossNonce, version]));
  ...
  const rows: RegistryRow[] = NETWORKS.map((n) => {
    ...
    const phase2TargetAddr = getCreate2Address(getAddress(phase1FactoryAddr), universalSalt, targetInitCodeHash);
    return {
      ...n,
  ...
  if (!argv['json-only']) {
    ...
    function printBlock(title: string, list: RegistryRow[]) {
      ...
      console.log('────────────────────────────────────────');
      list.forEach((r) => {
        console.log(`📍 ${r.network.padEnd(22)} (chain ${r.chainId.toString().padEnd(7)})  Factory: ${r.predictedFactoryAddress}  Target: ${r.predictedTargetAddress}`);
  ...
  if (!argv['json-only']) {
    ...
    console.log(`Factory cross-chain consistency: ${uniqFactories.length === 1 ? '✅ YES' : '❌ NO'}`);
    if (uniqTargets.length === 1) console.log(`🎯 Universal Target: ${getAddress(uniqTargets[0])}`);
    if (uniqFactories.length === 1) console.log(`🏗  Universal Factory: ${getAddress(uniqFactories[0])}`);
  }
  ...
  function toCSV(rows: RegistryRow[]) {
    ...
    for (const r of rows) lines.push([`"${r.network}"`, r.chainId, r.status, r.predictedFactoryAddress, r.predictedTargetAddress, r.explorerUrl].join(','));
    return lines.join('\n');
  }
  ...
  function toMD(rows: RegistryRow[]) {
    ...
    for (const r of rows) rowsOut.push(`| ${r.network} | ${r.chainId} | ${r.status} | ${r.predictedFactoryAddress} | ${r.predictedTargetAddress} | ${r.explorerUrl} |`);
    return rowsOut.join('\n');
  }
