...
async function main() {
  ...
  try {
    ...
    console.warn("ManifestDispatcher contract not found in artifacts. Ensure compiled artifacts exist.");
    process.exit(0);
  }
  ...
  try {
    ...
    try {
      ...
      console.warn('GasOptimizationUtils deploy/link skipped:', linkErr && linkErr.message ? linkErr.message : linkErr);
    }
    ...
    console.warn('ExampleFacetA not found or failed to deploy, falling back to deployer as facet (may revert)', err && err.message ? err.message : err);
  ...
  const leaf = ethers.utils.keccak256(
    ethers.utils.solidityPack(['bytes1', 'bytes4', 'address', 'bytes32'], ['0x00', selectors[0], facets[0], codehashes[0]])
  );

  // Commit the root to make applyRoutes callable (commitRoot requires COMMIT_ROLE which deployer has)
  try {
    const tx = await DispatcherFactory.deployTransaction.wait?.();
  } catch (e) {
    // noop - just ensuring deploy completed above
  }
  try {
