...
class AIUniversalASTChunker {
    ...
    async analyzeContract(filePath) {
        ...
        if (runtimeBytes <= this.maxRuntimeBytes) {
            return this.createSingleChunkAnalysis(filePath, content, stats.size, lines, abiData, runtimeBytes);
        }
    ...
    groupFunctionsByDomain(functions) {
        ...
        const views = functions.filter((f) => (f.stateMutability === "view" || f.stateMutability === "pure") && !core.includes(f));
        if (views.length)
            groups.push({ name: "View", functions: views, type: "view" });
        const state = functions.filter((f) => (f.stateMutability === "nonpayable" || f.stateMutability === "payable") &&
            !core.includes(f));
        if (state.length > 20) {
...
`;
    ...
    generateFunctionStub(f) {
        ...
        const vis = "external";
        const mut = f.stateMutability === "view" || f.stateMutability === "pure"
            ? f.stateMutability
            : f.stateMutability === "payable"
                ? "payable"
                : ""; // nonpayable => omit
        const mod = f.stateMutability === "view" || f.stateMutability === "pure" ? "" : " onlyDispatcher";
...
`;
    ...
    validateChunks(originalFns, chunks) {
        ...
        if (!selectorParity)
            errors.push("Selector parity failed");
        ...
        if (!runtimeSizeOk)
            errors.push("One or more facets exceed EIP-170");
        // loupe ban in facets
        const loupe = new Set(["0x1f931c1c", "0xcdffacc6", "0x52ef6b2c", "0xadfca15e"]);
        const bad = chunks.some((c) => c.type === "facet" && c.functions.some((f) => loupe.has(f.selector.toLowerCase())));
        const noLoupeInFacets = !bad;
        if (!noLoupeInFacets)
            errors.push("Loupe selectors found in a facet");
...
async function main(hre) {
    ...
    for (const rel of targets) {
        ...
        try {
            ...
            console.log(`✅ ${rel}: size ${(a.originalSize / 1024).toFixed(2)}KB, runtime ${a.originalRuntimeBytes} bytes`);
        }
        catch (e) {
