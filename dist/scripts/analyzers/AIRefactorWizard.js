...
function loadLoupeSelectors(root = process.cwd()) {
  try {
    ...
    if (fs_1.default.existsSync(p)) {
      ...
      if (j?.loupe_selectors) return j.loupe_selectors;
    }
  } catch {
    /* ignore */
  }
...
const selectorOf = (sig) =>
  '0x' + (0, ethers_1.keccak256)((0, ethers_1.toUtf8Bytes)(sig)).slice(2, 10);
    ...
    const facets = Object.entries(buckets)
      ...
      .map(({ name, fns }) => {
        ...
        return {
          ...
          codehash: (0, ethers_1.keccak256)((0, ethers_1.toUtf8Bytes)(`payrox.codehash.${name}`)),
        };
      });
