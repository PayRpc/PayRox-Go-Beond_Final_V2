...
class SolidityAnalyzer {
    ...
    async parseContract(sourceCode, contractName) {
        try {
            ...
            try {
                ...
                console.warn('Compilation failed, continuing with AST-only analysis:', e instanceof Error ? e.message : e);
                compiled = {};
            }
        ...
        program
            ...
            try {
                ...
                chunkPlan.chunks.forEach((chunk, i) => {
                    console.log(`  Chunk ${i + 1} (${chunk.id}): ${chunk.functions.length} functions, ~${chunk.estimatedSize} bytes, ${chunk.gasEstimate} gas`);
                });
    ...
    async refactorContract(sourceCode, options = {}) {
        ...
        for (const chunk of chunkPlan.chunks) {
            ...
            const funcs = chunk.functions.map((f) => `function ${f.name}(${(f.signature || '')}) external;`).join('\n');
    ...
    planChunksByFunction(functions, maxChunkSize) {
        ...
        for (const func of functions) {
            ...
            if (currentChunk.estimatedSize + func.estimatedSize >
                maxChunkSize) {
    ...
    detectFunctionFeature(func) {
        ...
        if (['transfer', 'approve', 'allowance', 'balanceof', 'totalsupply'].some(pattern => name.includes(pattern))) {
            return 'erc20';
        }
        // Access control functions
        if (['onlyowner', 'onlyadmin', 'onlyauthorized', 'require'].some(pattern => name.includes(pattern)) ||
        ...
        if (['mint', 'burn', 'pause', 'unpause', 'withdraw', 'emergency'].some(pattern => name.includes(pattern))) {
    ...
    calculateChunkOptimization(chunks, options) {
        ...
        if (dependencyScore > 0.3) {
            recommendations.push('High cross-chunk dependencies detected - consider reorganizing features');
        }
        if (chunks.length > 10) {
    ...
    checkManifestSecurity(analysis) {
        return {
            pausable: analysis.functions.some(func => func.name === 'pause' ||
                func.name === 'unpause' ||
                func.name === 'paused'),
    ...
    parseFunctionsLightweight(sourceCode) {
        ...
        for (const func of functions) {
            ...
            const mods = blob
                .replace(/\b(public|external|internal|private|pure|view|payable|virtual|override|returns)\b/g, ' ')
                .trim()
                .split(/\s+/)
                .filter(Boolean);
            func.modifiers = mods;
    ...
    isAdminFunctionLightweight(name, modifiers) {
        ...
        return (adminPatterns.some(rx => rx.test(name)) ||
    ...
    async parseContractUltraLightweight(sourceCode, contractName) {
        try {
            ...
            parsedFunctions.forEach((f, i) => {
                ...
                if (!fnSigs[f.name])
                    fnSigs[f.name] = f.signature;
                if (!fnSels[f.name])
                    fnSels[f.name] = f.selector;
            });
            // Complexity (heuristic): size + loops + write vs read mix
            const loopCount = (sourceCode.match(/\b(for|while)\s*\(/g) || []).length;
            const writeCount = parsedFunctions.filter(f => f.stateMutability !== 'view' && f.stateMutability !== 'pure').length;
            ...
            const readFns = parsedFunctions
                .filter(f => !adminFns.includes(f.name) &&
            ...
            const warnings = [];
            if (sizeBytes > RUNTIME_LIMIT)
                warnings.push(`Source size hint ${sizeBytes}B exceeds EIP-170 runtime limit (${RUNTIME_LIMIT}B).`);
