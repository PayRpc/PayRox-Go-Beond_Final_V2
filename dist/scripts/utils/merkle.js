...
function encodeLeaf(selector, facet, codehash) {
  ...
  const packed = (0, ethers_1.solidityPacked)(
...
function deriveSelectorsFromAbi(abi) {
  ...
  const sels = sigs.map((sig) =>
    (0, ethers_1.keccak256)((0, ethers_1.toUtf8Bytes)(sig)).slice(0, 10),
  );
  ...
  for (const file of Object.keys(linkReferences)) {
    ...
    for (const libName of Object.keys(libs)) {
      ...
      for (const ref of libs[libName]) {
        const start = ref.start; // bytes offset
        const length = ref.length; // bytes length, expected 20
...
async function generateManifestLeaves(manifest, artifacts, factoryAddress, opts) {
  if (!factoryAddress) {
    ...
    const allHaveAddresses =
      ...
      manifest.facets.every((f) => !!f.address);
    if (!allHaveAddresses) {
      throw new Error(
  ...
  for (const facet of manifest.facets) {
    ...
    const creation = linkBytecode(art.bytecode, art.linkReferences, opts?.libraryAddresses);
    ...
    if (explicitAddress) {
      ...
      const salt =
        explicitSalt ?? (0, ethers_1.keccak256)((0, ethers_1.toUtf8Bytes)(`PayRox-${facet.name}`));
