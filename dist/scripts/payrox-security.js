...
function parseArgs() {
  ...
  const mythrilTargets = (mythTargetsArg ? JSON.parse(mythTargetsArg) : null) || [
    ...
    {
      file: 'contracts/ai-refactored-facets/GovernanceCoreFacet.sol',
      solv: '0.8.20',
      name: 'governance',
    },
  ];
...
class PayRoxSecurityAnalyzer {
  ...
  async checkDeps() {
    ...
    console.log(
      this.hasSlither
        ? '✅ Slither available'
        : '⚠️ Slither not found (pip install slither-analyzer)',
    );
    this.hasMythril = tryCmd('myth version');
    console.log(
      this.hasMythril ? '✅ Mythril available' : '⚠️ Mythril not found (pip install mythril)',
    );
  }
  async compile() {
    console.log('\n🔧 Compiling (Hardhat)...');
  ...
  parseMythril(file, component) {
    try {
      ...
      for (const is of issues) {
        ...
        const sev =
          String(is.severity || 'INFO').toUpperCase() in { HIGH: 1, MEDIUM: 1, LOW: 1, INFO: 1 }
  ...
  writeReports() {
    ...
    fs.writeFileSync(
      path.join(this.args.outDir, 'security-summary.json'),
...
if (require.main === module) {
  ...
  new PayRoxSecurityAnalyzer(args).run().catch((e) => {
    console.error(e);
    process.exit(1);
  });
}
