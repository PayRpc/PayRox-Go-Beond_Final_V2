...
const NETWORKS = [
    ...
    { network: "Fantom Opera", chainId: 250, explorerUrl: "https://ftmscan.com", status: "production", create2Deployer: EIP2470 },
    ...
    // Tier 3
    { network: "Moonbeam", chainId: 1284, explorerUrl: "https://moonbeam.moonscan.io", status: "production", create2Deployer: EIP2470 },
    { network: "Cronos", chainId: 25, explorerUrl: "https://cronoscan.com", status: "production", create2Deployer: EIP2470 },
    { network: "Aurora", chainId: 1313161554, explorerUrl: "https://aurorascan.dev", status: "production", create2Deployer: EIP2470 },
    { network: "Metis Andromeda", chainId: 1088, explorerUrl: "https://explorer.metis.io", status: "production", create2Deployer: EIP2470 },
    { network: "Boba Network", chainId: 288, explorerUrl: "https://bobascan.com", status: "production", create2Deployer: EIP2470 },
    { network: "Moonriver", chainId: 1285, explorerUrl: "https://moonriver.moonscan.io", status: "production", create2Deployer: EIP2470 },
    { network: "Fuse", chainId: 122, explorerUrl: "https://explorer.fuse.io", status: "production", create2Deployer: EIP2470 },
    { network: "Harmony One", chainId: 1666600000, explorerUrl: "https://explorer.harmony.one", status: "production", create2Deployer: EIP2470 },
    // Testnets (Mumbai is legacy; prefer Amoy)
    { network: "Ethereum Sepolia", chainId: 11155111, explorerUrl: "https://sepolia.etherscan.io", status: "testnet", create2Deployer: EIP2470 },
    { network: "Polygon Amoy", chainId: 80002, explorerUrl: "https://www.oklink.com/amoy", status: "testnet", create2Deployer: EIP2470 },
    { network: "Arbitrum Sepolia", chainId: 421614, explorerUrl: "https://sepolia.arbiscan.io", status: "testnet", create2Deployer: EIP2470 },
    { network: "Base Sepolia", chainId: 84532, explorerUrl: "https://sepolia.basescan.org", status: "testnet", create2Deployer: EIP2470 },
    { network: "Optimism Sepolia", chainId: 11155420, explorerUrl: "https://sepolia-optimism.etherscan.io", status: "testnet", create2Deployer: EIP2470 },
    // Local
...
function hex32(x) {
    ...
    if (!x.startsWith("0x"))
        throw new Error("salt/bytes must be 0x-prefixed");
    const n = x.length;
    if (n === 66)
...
async function loadArtifactInfo(name) {
    try {
...
async function main() {
    ...
    if (!targetInitCode) {
        ...
        if (fbInfo?.bytecode) {
            ...
            console.warn(`⚠️  Target artifact '${targetArtName}' missing; found fallback artifact '${fallback}'. allowFallback=${allow}`);
            if (!allow) {
                throw new Error(`Target artifact '${targetArtName}' missing and ALLOW_FALLBACK is not set. Set PRX_TARGET_BYTECODE or ALLOW_FALLBACK=true to permit fallback.`);
            }
    ...
    function validateInitCode(name, initCode) {
        if (!initCode)
            throw new Error(`${name} init code missing. Set PRX_${name.toUpperCase()}_BYTECODE or compile the contract.`);
        if (!initCode.startsWith("0x"))
            throw new Error(`${name} init code must be 0x-prefixed`);
    ...
    const defaultFactorySalt = keccak256(solidityPacked(["string", "string"], ["PayRoxFactory", version]));
    ...
    const universalSalt = keccak256(solidityPacked(["string", "string", "uint256", "string"], ["PayRoxCrossChain", content, crossNonce, version]));
    ...
    const rows = NETWORKS.map((n) => {
        const phase1FactoryAddr = getCreate2Address(getAddress(n.create2Deployer), factorySalt, factoryInitCodeHash);
    ...
    console.log("────────────────────────────────────────");
    prod.forEach((r) => {
        console.log(`📍 ${r.network.padEnd(22)} (chainId ${r.chainId.toString().padEnd(7)})  Factory: ${r.predictedFactoryAddress}  Target: ${r.predictedTargetAddress}`);
