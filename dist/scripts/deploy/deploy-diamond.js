...
class DiamondDeployer {
    ...
    async deployFacets() {
        ...
        for (const [facetName, facetData] of Object.entries(this.manifest.facets)) {
            ...
            const facetSalt = hardhat_1.ethers.utils.keccak256(hardhat_1.ethers.utils.toUtf8Bytes(`${this.config.salt}-${facetName}`));
    ...
    async deployDiamond() {
        ...
        const diamond = await Diamond.deploy(deployer.address, diamondCutFacet.address, { gasLimit: 5000000 });
        await diamond.deployed();
        console.log(`  ✅ Diamond deployed to: ${diamond.address}`);
        // Add loupe facet
        const diamondCut = await hardhat_1.ethers.getContractAt("contracts/interfaces/IDiamondCut.sol:IDiamondCut", diamond.address);
        const loupeSelectors = this.getFunctionSelectors(diamondLoupeFacet);
        await diamondCut.diamondCut([{
                facetAddress: diamondLoupeFacet.address,
                action: 0, // Add
                functionSelectors: loupeSelectors
            }], hardhat_1.ethers.constants.AddressZero, "0x");
        return diamond;
    }
    async initializeDiamond(diamond) {
        ...
        // Execute diamond cut
        const tx = await diamondCut.diamondCut(facetCuts, hardhat_1.ethers.constants.AddressZero, "0x", { gasLimit: 8000000 });
        await tx.wait();
        console.log('  ✅ All facets added to Diamond');
    }
    async verifyDeployment(diamond) {
        console.log('✅ Verifying deployment...');
        const diamondLoupe = await hardhat_1.ethers.getContractAt("contracts/interfaces/IDiamondLoupe.sol:IDiamondLoupe", diamond.address);
    ...
    async updateManifest(diamond) {
        ...
        for (const [facetName, facetData] of Object.entries(this.manifest.facets)) {
            for (const selector of facetData.selectors) {
                const leaf = hardhat_1.ethers.utils.keccak256(hardhat_1.ethers.utils.defaultAbiCoder.encode(["bytes4", "address", "bytes32"], [selector, facetData.address, hardhat_1.ethers.utils.keccak256(hardhat_1.ethers.utils.toUtf8Bytes(facetName))]));
                leaves.push(leaf);
            }
        }
        // Simple merkle root calculation (in production, use proper merkle tree)
        const combinedHash = hardhat_1.ethers.utils.keccak256(hardhat_1.ethers.utils.concat(leaves.sort()));
