...
function findCollisions(selectors, labels) {
    ...
    selectors.forEach((sel, i) => {
        if (seen[sel])
            coll.push(`collision:${sel} => ${labels[i]} vs ${seen[sel]}`);
        else
            seen[sel] = labels[i];
    });
    return coll;
    ...
    try {
        ...
        const fns = Array.isArray(functions)
            ...
            : Array.isArray(analyzer?.functions)
                ...
                : [];
        if (fns.length === 0)
            return res.status(400).json({ error: "No functions provided" });
        ...
        for (const f of fns) {
            ...
            if (mut === "view" || mut === "pure" || n.startsWith("get") || n.startsWith("facet"))
                groups.View.push(f);
            else if (n.includes("owner") || n.includes("admin") || n.includes("pause") || n.includes("govern"))
                groups.Admin.push(f);
            else if (n.includes("oracle") || n.includes("twap") || n.includes("price") || n.includes("util"))
...
app.post("/api/manifest", (req, res) => {
    try {
        ...
        const leaves = routes.map((r) => (0, ethers_1.keccak256)(abi.encode(["bytes4", "address"], [r.selector, r.facet])));
