...
export class AIUniversalASTChunker {
  ...
  private async loadAbi(contractPath: string): Promise<{
    ...
    for (const fragment of functionFragments) {
      try {
        const fnName: string = fragment.name;
        const inputs: any[] = fragment.inputs || [];

        const signature = `${fnName}(${inputs.map((i: any) => i.type || '').join(',')})`;

        // Compute selector via keccak256(name(types)) and take first 4 bytes
        ...
        console.warn(`AI chunker: skipped function fragment in ${contractName}:`, err);
      }
    }

    return { name: contractName, artifact, iface, functions: fns };
  }
