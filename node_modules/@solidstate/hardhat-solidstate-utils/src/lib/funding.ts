...
const defaultFundingRequestData: FundingRequestData = {
  requestedAt: 0,
  cooldown: 0,
};

// TODO: set up multisig
const donationAddresses: { [chainId: string]: string } = {
  '0x7a69': '0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266',
};

export const donateNativeToken = async (
  ...
  const response = await hre.interruptions.requestInput(
    pkg.name,
    `Sending ${amount} native tokens (${value} wei) from account ${from}; proceed? (y/n)`,
  );
...
export const delay = async (timeout: number = 10, delay: number = 0) => {
  await sleep(delay);
...
const readFundingRequestData = async (): Promise<FundingRequestData> => {
  try {
...
const writeFundingRequestData = async (data: FundingRequestData) => {
  const filePath = getFundingDataFilePath();
