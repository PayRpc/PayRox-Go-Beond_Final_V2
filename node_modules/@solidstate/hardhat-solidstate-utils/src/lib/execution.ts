...
import type { HardhatPackageJson } from '../types.js';
...
export const isHardhatCLIRunning = async () => {
  try {
    ...
    const packageJson = await readJsonFile<HardhatPackageJson>(packageJsonPath);

    const argvPath = await getRealPath(process.argv[1]);
    const hardhatCliPath = await getRealPath(
      path.resolve(path.dirname(packageJsonPath), packageJson.bin.hardhat),
    );
