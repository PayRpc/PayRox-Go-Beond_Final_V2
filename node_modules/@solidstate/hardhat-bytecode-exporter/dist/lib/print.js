...
export const printBytecode = (contractNameOrFullyQualifiedName, bytecode, deployedBytecode) => {
    const table = createTable({
        colWidths: [38, 38],
    });
    // TODO: format fully qualified name
    const bytecodeSize = Buffer.from(bytecode.replace(/__\$\w*\$__/g, '0'.repeat(40)).slice(2), 'hex').length;
    const deployedBytecodeSize = Buffer.from(deployedBytecode.replace(/__\$\w*\$__/g, '0'.repeat(40)).slice(2), 'hex').length;
