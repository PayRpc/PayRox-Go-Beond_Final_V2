...
export const exportBytecode = async (context, configEntries) => {
    await clearBytecode(context, configEntries);
    await Promise.all(configEntries.map((entry) => exportBytecodeGroup(context, entry)));
};
const exportBytecodeGroup = async (context, config) => {
    ...
    const outputData = artifacts
        ...
        const destination = path.resolve(outputDirectory, config.rename(sourceName, contractName)) +
            '.init.bin';
        const deployedDestination = path.resolve(outputDirectory, config.rename(sourceName, contractName)) +
            '.deployed.bin';
        return [
            {
    ...
    await Promise.all(outputData.map(({ destination, contents }) => writeUtf8File(destination, contents)));
};
