import { exists, getAllFilesMatching, readUtf8File, remove, } from '@nomicfoundation/hardhat-utils/fs';
import path from 'node:path';
export const clearBytecode = async (context, configEntries) => {
    const entries = configEntries.filter((entry) => entry.clear);
    await Promise.all(entries.map((entry) => clearBytecodeGroup(context, entry)));
};
const clearBytecodeGroup = async (context, config) => {
    ...
    const binFiles = await getAllFilesMatching(outputDirectory, (absolutePathToFile) => path.extname(absolutePathToFile) === '.bin');
    // validate file contents and delete
    await Promise.all(binFiles.map(async (file) => {
